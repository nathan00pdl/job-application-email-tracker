...
- **Scheduling / execution:** GitHub Actions (`schedule` cron trigger, daily at 02:17 São Paulo time — early, because GitHub starts scheduled runs several hours late and the digest should still arrive early in the morning), no always-on server
...
The template is `resumo_diario_lista`, registered under the `en` language tag, with ten blanks: the period, one line with the total and the counts by kind (*"14 e-mails: 2 entrevistas, 10 confirmações de inscrição"*), six places for the emails that wait on the reader, how many more there are, and a link to the spreadsheet. `DigestMessage` fills them. A value cannot hold a line break, so each email gets a place of its own in the fixed text; one template serves every day, and an unused place holds a dash. The spreadsheet id is read from the sync's own setting, so the link can never point at a different sheet.

Meta refuses a message longer than 1,024 characters once the blanks are filled, counting the fixed text with them — an earlier template, which explained the format in three paragraphs, left no room for a single email and was refused outright. `DigestMessage` keeps the template's text and measures what it is about to send: the listed emails give way one at a time, counted in "além desses", and then the counts by kind, leaving how many arrived. The fixed text is deliberately short, so that six emails with their links still fit.
