...
A backend system that scans a personal Gmail inbox once a day, identifies emails related to job applications, classifies them using a rules filter combined with an LLM (Claude), persists the results as the source of truth in a PostgreSQL database, mirrors them into a Google Sheet used as a manual control dashboard, and sends a daily WhatsApp summary.
...
- **External integrations:** Gmail API, Anthropic API (Claude), Google Sheets API, Meta WhatsApp Cloud API
...
The system is fundamentally an orchestrator around external integrations (Gmail, Claude, Postgres, Sheets, WhatsApp), so a plain layered (controller/service/repository) structure would blur business logic with integration details. Hexagonal architecture keeps the domain free of framework and vendor concerns:

- **`domain`** — core models and business rules. No framework dependencies, no knowledge of Gmail/Claude/Postgres/Sheets/WhatsApp. Contains the classification decision rule (see below) and the digest-building logic.
...
- **`ports`** — interfaces the domain/application layer depends on:
  ...
  - `ClassifierPort` — classify a single email into a structured result
...
- **`adapters`** — concrete implementations of each port:
  ...
  - `adapters/claude` → `ClaudeApiAdapter implements ClassifierPort`
...
5. New emails pass through a cheap rule filter (known ATS/job-platform domains and keywords). Only matches proceed to LLM classification, to control cost and volume.
6. `ClaudeApiAdapter` classifies each candidate into a structured result (company, role, platform, update type, summary, urgency). Email content is treated strictly as untrusted input — its output only ever populates data fields, never triggers behavior directly (mitigates prompt injection risk).
7. The classification decision rule is applied (see below) and the record is persisted first, in Postgres — the source of truth.
8. New/unsynced records (`sheet_synced_at IS NULL`) are written to the Google Sheet.
9. A WhatsApp message is sent with the daily summary — every day, even when there is nothing new.
10. If any external service is unreachable and prevents completion (not a per-email classification failure), the job fails visibly: GitHub Actions' built-in failure email fires automatically, and a final step sends a separate WhatsApp alert using a distinct message template.
11. The runner is destroyed. Nothing stays running between executions.
...
Two independent signals are recorded per email:

- `matched_rule_filter` — whether the cheap pre-filter flagged it
- `llm_classified_relevant` — whether Claude classified it as related to a job application

The LLM's classification is authoritative for `included_in_digest` (whether it counts in the run and appears in the WhatsApp summary). When the two signals disagree — most notably when the rule filter matched but the LLM says "not related" — the record is still persisted (not discarded) and flagged with `has_disagreement = true`, surfaced with a visual highlight in the Google Sheet for manual review. This keeps the LLM as the single source of truth while preserving an audit trail for classifier drift.
...
## Database schema (initial migration — `V1__create_email_classifications.sql`)
...
CREATE INDEX idx_email_classifications_has_disagreement ON email_classifications (has_disagreement);
```
...
- **Secrets:** OAuth tokens, service account credentials, the Neon connection string, the Anthropic API key, and the WhatsApp access token are never committed. They live in GitHub Secrets and are injected as environment variables at runtime. GitHub Secret Scanning + push protection is enabled on the repository.
...
- **Prompt injection:** email content is untrusted input. The classifier's output is always treated as structured data (populating typed fields), never as an instruction the application executes — this containment is a deliberate design decision, not incidental.
...
- **Unit tests (JUnit 5 + Mockito + AssertJ):** the domain/application layer (classification decision rule, digest building) is tested with no mocking framework needed, since it only depends on port interfaces — fakes are enough. The `ClaudeApiAdapter`'s parsing/mapping logic is tested with Mockito simulating the Anthropic API response.
...
- **Out of scope for now:** end-to-end tests hitting the real external APIs (Gmail, Claude, WhatsApp, Sheets) in CI — this would require production credentials in CI for limited benefit; confidence in the full pipeline comes from the actual daily run instead.
